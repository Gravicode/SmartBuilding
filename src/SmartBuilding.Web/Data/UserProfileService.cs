            ...
            try
            {
                ...
                foreach (var row in worksheet.Rows)
                {
                    if (counter > 0)
                    {
                        ...
                        newUserProfile.Username = string.IsNullOrEmpty(row.Cells[10].Value?.ToString().Trim()) ? MailHelper.GenerateEmailFromName(newUserProfile.FullName.Trim(), "SmartBuilding.online") : row.Cells[10].Value?.ToString().Trim();
